...
save(NUTS_info_list, file = "clean_NUTS_info.csv")
...
models <- stan_demo(0) # choose option 1
source('test_ADVI.R')
load("clean_NUTS_info.csv")

ADVI_report <- test_ADVI(NUTS_info_list, c("meanfield", "fullrank"))
cat("Results saved in ADVI_report_info.csv")
save(ADVI_report, file = "ADVI_report_info.csv")